		...
		while ($row=mysqli_fetch_row($result))
	   	{
			if($bid==$row[15])
			{
				...
				if($r[1]>0)
				{
					...
					mysqli_query($conn, $sql2);
					$sql2 = "INSERT INTO confirmed_booking VALUES ('$row[0]','$row[1]','$row[2]','$row[3]','$row[4]','$row[5]','$row[6]','$row[7]','$row[8]','$row[9]','$row[10]','$row[11]','$row[12]','Confirmed','$row[14]','$row[15]','$row[16]')";
					mysqli_query($conn, $sql2);
